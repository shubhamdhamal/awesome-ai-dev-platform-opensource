...
const ImportFlowDialog = (
  ...
  const handleFileChange = async (
    ...
    const convertedTemplate = await convertTemplate(newTemplates);
    setIsTemplateLoading(false);
    setTemplates(convertedTemplate);
  };
  ...
  const convertTemplate = async (originalTemplate: FlowTemplate[]) => {
    const templatesWithPossibleNulls = await Promise.all(originalTemplate.map(async (template) => {
      console.log(template);
      const templateType = checkTemplateType(template);
      console.log('templateType', templateType);
  
      try {
        if (templateType === ImportTemplateType.N8N) {
          return convertN8n(template);
        } else if (templateType === ImportTemplateType.MAKE) {
    ...
    return templatesWithPossibleNulls.filter((template): template is FlowTemplate => template !== null);
  };
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <DialogFooter>
          <Button
            ...
            disabled={isPending || isTemplateLoading}
          >
            {t('Cancel')}
          </Button>
          <Button onClick={handleSubmit} loading={isPending || isTemplateLoading}>
            {t('Import')}
          </Button>
        </DialogFooter>
      </DialogContent>
