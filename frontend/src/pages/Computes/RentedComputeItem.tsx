...
export type TProps = {
  ...
  onNotificationCompute?: (id: number) => void,
  history_id: number
}
...
export default function RentedComputeItem(props: TProps) {
    ...
    const notificationButton = (
      <>
        <Button
          ...
          onClick={() => props.onNotificationCompute?.(props.compute_id)}
        />
      </>
    );
  ...
  }, [navigate, props, publish, user?.uuid]);

  return (
    <div className="p-my-computes-compute">
      <div className="p-my-computes-compute__inner">
        <div className="badges">
          ...
          {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox /> : <span className="badge badge--purple" id={tooltipPrefix + "machine-type"}>
            {props.machine_type ?? "??????"}
          </span>}
          ...
          {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox /> : <span className="badge badge--black">Service: {props.service ?? "?"}</span>}
          {props?.type !== "COMPUTE-SELF-HOST" &&
            <>
              {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox /> : <span className="badge badge--semi-blue">{getRentedGpuStatusText(props.source)}</span>}
            </>
          }
          {props.price &&
            <>
              {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox /> : <span className="badge badge--price" id={tooltipPrefix + "price"}>{props.price ?? "?"}</span>}
            </>
          }
          {props.price &&
        ...
        <div className="columns">
          <div className="column column--gpu">
            ...
            <div className="split">
              ...
              {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox style={{ marginLeft: "5px" }} /> : <div className="right">
                <div className="gpu-ram" id={tooltipPrefix + "gpu-ram"}>{formatBytesToGB(props.per_gpu_ram) ?? "?"} GB</div>
          ...
          <div className="column column--cpu">
            {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox style={{ marginBottom: "10px" }} /> : <div className="motherboard" id={tooltipPrefix + "motherboard"}>{props.motherboard ?? "?"}</div>}
            <Tooltip place="top-start" positionStrategy="fixed" content="Motherboard"
              anchorSelect={"#" + tooltipPrefix + "motherboard"} />
            {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox style={{ marginBottom: "10px" }} /> : <div className="pcie">{props.number_of_pcie_per_gpu ?? "?"}</div>}
            {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox style={{ marginBottom: "10px" }} /> : <div className="cpu" id={tooltipPrefix + "cpu"}>{props.cpu ?? "?"}</div>}
          ...
          <div className="column column--network">
            {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox style={{ marginBottom: "10px" }} /> : <div className="upload" id={tooltipPrefix + "upload"}>
            ...
            {isInstalling || isWaitVerify || isWaitCrypto ? <SkeletonBox style={{ marginBottom: "10px" }} /> : <div className="download" id={tooltipPrefix + "download"}>
              <svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
