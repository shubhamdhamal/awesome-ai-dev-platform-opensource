...
export default function FineTuneAndDeployLayout({children, onBack, onSkip, onNext, withoutSteps, bgColor}: TProps) {
      ...
      return [
        {
          ...
          icon: <IconData />,
          onClick: project ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/setup-infrastructure/storage") : undefined,
        },
        {
          label: "GPU for training and deploying",
          activeChecker: () => urlParts[4] === "gpu",
          icon: <IconCpu />,
          onClick: project ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/setup-infrastructure/gpu") : undefined,
        },
      ];
    }
      ...
      return [
        {
          ...
          onClick: project ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/data-preparation/local-upload") : undefined,
        },
        {
  ...
  const isDataPage = useMemo(() => urlParts.length > 3 && urlParts[3] === "data", [urlParts]);
  ...
  useEffect(() => {
    ...
    // Existing project: Cancel to data
      ...
      if (permission.configure) {
        actions.push({label: "Go to Settings", actionType: "primary", onClick: () => navigate("/fine-tune-and-deploy/" + project?.id + "/settings")})
      }
    ...
    // Data page has its own actions and close callback
      ...
      if (permission.configure) {
        ...
        actions.push({label: "Settings", actionType: isSettings ? "primary" : undefined, onClick: () => navigate("/fine-tune-and-deploy/" + project?.id + "/settings")});
      }


      const isTraining = urlParts.length > 3 && urlParts[3] === "training-dashboard";
      actions.push({label: "Training Dashboard", actionType: isTraining ? "primary" : undefined, onClick: () => navigate("/fine-tune-and-deploy/" + project?.id + "/training-dashboard")});

      const isDeploy = urlParts.length > 3 && urlParts[3] === "demo-and-deploy";
      actions.push({label: "Demo and Deploy", actionType: isDeploy ? "primary" : undefined, onClick: () => navigate("/fine-tune-and-deploy/" + project?.id + "/demo-and-deploy")});
  ...
  return (
    <FlowLayout
      primarySteps={isShowWorkflow || withoutSteps ? undefined : [
        {
          label: "Setup Project",
          activeChecker: () => url.startsWith("/fine-tune-and-deploy/create") || urlParts[3] === "project",
          onClick: project ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/project") : undefined,
        },
        {
        ...
        },
        {
          ...
          onClick: project ? () => navigate("/fine-tune-and-deploy/" + project?.id + "/setup-model") : undefined,
        },
      ]}
